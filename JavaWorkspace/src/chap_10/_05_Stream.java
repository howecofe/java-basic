...
public class _05_Stream {
    public static void main(String[] args) {
        ...
        String[] lengs = {"python", "java", "javascript", "c", "c++", "c#"};
        Stream<String> lengStream = Arrays.stream(lengs);
...
//        System.out.println(langList.size());
        ...
        // Stream.of()
        Stream<String> lengListOfStream = Stream.of("python", "java", "javascript", "c", "c++", "c#");

        // 스트림 사용 : 원본 데이터에 영향을 주지 않는다.
...
//        Arrays.stream(scores).filter(x -> x >= 90).forEach(System.out::println);
        ...
        // 4글자 이하의 언어를 정렬해서 출력
//        Arrays.stream(lengs).filter(x -> x.length() <= 4).sorted().forEach(System.out::println);
